...
output_file = '%s_Phylum_to_Family.csv' % (release_version)
output_file = '%s_Phylum_to_Genus.csv' % (release_version)
...
for each in open(GTDB_output):
    if not each.startswith('user_genome'):
        ...
        store_list.append('%s,%s' % (f, g))

        # if (p.endswith('__') is False) and (c.endswith('__') is False):
        #     store_list.append('%s,%s' % (p, c))
        #
        # if (c.endswith('__') is False) and (o.endswith('__') is False):
        #     store_list.append('%s,%s' % (c, o))
        #
        # if (o.endswith('__') is False) and (f.endswith('__') is False):
        #     store_list.append('%s,%s' % (o, f))
        #
        # if (f.endswith('__') is False) and (g.endswith('__') is False):
        #     store_list.append('%s,%s' % (f, g))
...
print(store_list_uniq_count_dict)
...
# run Rscript
os.system('Rscript %s -f %s -x 1000 -y 1200' % (get_sankey_plot_Rscript, output_file))
